# Claude Code Project Instructions — syntect-kotlin

## Project Overview

This is **syntect-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`syntect`](https://crates.io/crates/syntect).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/syntect/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/syntect>
